...
const Saveti = () => {
    ...
    return (
        <div className="min-h-screen bg-background font-sans">
            ...
            {/* HERO SECTION */}
            {/* ═══════════════════════════════════════════════════════════════════════════ */}
            <section className="pt-32 pb-20 px-4 text-center bg-secondary relative overflow-hidden">
                ...
                <div className="container mx-auto relative z-10">
                    <h1
                        className="text-6xl md:text-8xl font-heading font-bold text-foreground mb-6 opacity-0 translate-y-8"
                        style={{
                    ...
                    <p
                        className="text-xl md:text-2xl text-muted-foreground max-w-3xl mx-auto mb-12 opacity-0"
                        style={{
                    ...
                    >
                        ...
                        <Button
                            ...
                            className={`text-lg px-8 py-6 font-heading uppercase tracking-wider transition-all duration-200 hover:scale-103 ${activeCategory === null
                        ...
                        {["Brada", "Frizure", "Proizvodi"].map((cat) => (
                            <Button
                                ...
                                className={`text-lg px-8 py-6 font-heading uppercase tracking-wider transition-all duration-200 hover:scale-103 ${activeCategory === cat.toUpperCase()
                                    ? "bg-gold text-black hover:bg-gold-dark shadow-lg shadow-gold/20"
                                    : "border-gold/50 text-gold hover:bg-gold hover:text-black"
                                    }`}
            ...
            {/* QUICK TIPS SLIDER */}
            {/* ═══════════════════════════════════════════════════════════════════════════ */}
            <section className="py-6 bg-gold/5 border-y border-gold/10 overflow-hidden">
                <div className="flex animate-marquee">
                    {[...quickTips, ...quickTips].map((tip, i) => (
                        <div key={i} className="flex-shrink-0 px-12 text-muted-foreground text-lg whitespace-nowrap">
                            {tip}
                        </div>
                    ))}
                </div>
            ...
            <section className="py-20 px-4">
                <div className="container mx-auto">
                    <div className="text-center mb-16">
                        <span className="inline-block px-4 py-2 bg-gold/10 border border-gold/20 rounded-full text-gold text-sm font-semibold mb-4">
                            SAVETI & EDUKACIJA
                        </span>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {filteredTips.map((tip, index) => (
                            <div
                                ...
                                className={`group bg-card border border-border rounded-2xl p-8 transition-all duration-400 cursor-pointer
                  ...
                  hover:-translate-y-1.5 hover:shadow-2xl hover:shadow-gold/15 hover:border-gold/30`}
                            ...
                            >
                                ...
                                {/* Title */}
                                <h3 className="text-2xl font-heading font-bold text-foreground mb-4 group-hover:text-gold transition-colors duration-300">
                                    {tip.title}
                                </h3>
            ...
            <section className="py-24 px-4 bg-secondary relative overflow-hidden">
                ...
                <div className="container mx-auto relative z-10">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
                                ...
                                <div className="absolute inset-0 flex items-center justify-center p-12">
                                    <div className="text-center">
                                        <div className="w-24 h-24 bg-gold/10 rounded-full flex items-center justify-center mx-auto mb-8 text-gold">
                                            <span className="text-4xl font-heading font-bold">{currentTutorialStep + 1}</span>
                                        </div>
                        ...
                        {/* Right: Tutorial Info */}
                        <div>
                            <span className="inline-block px-4 py-2 bg-gold/10 border border-gold/20 rounded-full text-gold text-sm font-semibold mb-6">
                                TUTORIAL
                            </span>
                            ...
                            <div className="space-y-4">
                                {tutorialSteps.map((step, i) => (
                                    <div
                                        key={i}
                                        className={`flex items-start gap-4 p-4 rounded-xl transition-all duration-300 ${i === currentTutorialStep ? 'bg-gold/10 border border-gold/20' : 'bg-transparent'
                                            }`}
                                    >
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 font-bold text-sm ${i === currentTutorialStep ? 'bg-gold text-black' : 'bg-muted text-muted-foreground'
                                            }`}>
                                            {step.step}
                                        </div>
                                        <p className={`text-lg ${i === currentTutorialStep ? 'text-foreground' : 'text-muted-foreground'}`}>
                                            {step.text}
                                        </p>
                                    </div>
                                ))}
            ...
            <section id="products" className="py-24 px-4">
                <div className="container mx-auto">
                    <div className="text-center mb-16">
                        <span className="inline-block px-4 py-2 bg-gold/10 border border-gold/20 rounded-full text-gold text-sm font-semibold mb-4 animate-fade-in">
                            DOSTUPNO U SALONU
                        </span>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {products.map((product, index) => (
                            <div
                                key={index}
                                className="group bg-card border border-border rounded-2xl p-8 text-center 
